...
router.get("/catalog", BookController.showBookCatalog);
...
router.get("/logout", UserController.doLogout, (req, res) => {
    //req.session.destroy() //καταστρέφουμε τη συνεδρία στο session store // EDIT έχει ήδη καταστραφεί στο doLogout
    res.redirect("/")
})
